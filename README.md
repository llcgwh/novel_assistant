...
15. **☁️ WebDAV 多终端同步** ✨ 新增
    ...
    - 管理云端备份文件（查看/删除）
    - 自动同步开关（计划中）
    - 已保存密码不重复要求输入；接口与 JSON 导出不返回密码（数据库加密存储待实现）
...
恢复会替换当前小说的关联数据。旧版备份仍可导入，但其中没有的关系组不会被保留。图片目前仅保存地址，**没有打包图片文件**；请另外备份后端 uploads 目录。详见 [分批修复记录](docs/maintenance-batches.md)。
